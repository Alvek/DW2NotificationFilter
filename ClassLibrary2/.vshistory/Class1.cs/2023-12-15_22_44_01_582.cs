...
namespace ClassLibrary2
{
    ...
    public class NotificationFilterPatcher
    {
        ...
        public static void Postfix(ref bool __result, EmpireMessage __message)
        {
            //__result = false;
            //return;
            bool res = true;
            ...
            if (__result)
            {
                ...
                {
                    if (item.MessageType == __message.Type)
                    {
                        res = false;
                        break;
                    }
                    else
                    {
                        ...
                        {
                            if (__message.RelatedItem != null && (itemRule.RelatedItemType == __message.RelatedItem.Type &&
                                itemRule.Id == __message.RelatedItem.Id))
                            {
                                res = false;
                                break;
                            }
                        ...
                        {
                            var test1 = __message.RelatedObject;
                            var test2 = __message.RelatedObject.ItemType;
                            var test3 = itemRule.RelatedObjectType;

                            if (__message.RelatedObject != null && (itemRule.RelatedObjectType == __message.RelatedObject.ItemType))
                            {
                                res = false;
                                break;
                            }
                ...
                if (!res)
                {
                    ...
                    if (activatedRule.DefaultAction == DefaultMessageAction.Yes)
                    {
                        ...
                        DWGamePatcher._Renderer.EmpireMessageYesClick(null, new DWEventArgs(__message));
                    }
                    else if (activatedRule.DefaultAction == DefaultMessageAction.No)
                    { DWGamePatcher._Renderer.EmpireMessageNoClick(null, new DWEventArgs(__message)); }
                    else if (activatedRule.DefaultAction == DefaultMessageAction.Show)
                    { DWGamePatcher._Renderer.EmpireMessageShowClick(null, new DWEventArgs(__message)); }
                }
            }
        }
