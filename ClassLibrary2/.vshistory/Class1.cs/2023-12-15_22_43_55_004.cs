            ...
            if (__result)
            {
                ...
                foreach (var item in _Rules.RuleList)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var test1 = __instance.RelatedObject;
                            var test2 = __instance.RelatedObject.ItemType;
                ...
                if (!res)
                {
                    ...
                    if (activatedRule.DefaultAction == DefaultMessageAction.Yes)
                    {
                        ...
                        DWGamePatcher._Renderer.EmpireMessageYesClick(null, new DWEventArgs(__instance));
                    }
                    else if (activatedRule.DefaultAction == DefaultMessageAction.No)
                    { DWGamePatcher._Renderer.EmpireMessageNoClick(null, new DWEventArgs(__instance)); }
                    else if (activatedRule.DefaultAction == DefaultMessageAction.Show)
                    { DWGamePatcher._Renderer.EmpireMessageShowClick(null, new DWEventArgs(__instance)); }
                }
            }
        }
