...
namespace ClassLibrary2
{
    ...
    //разобраться почему не находит метод для патча, пишут что мне нужен TargetMethod()
    //    https://github.com/pardeike/Harmony/issues/85
    [HarmonyDebug]
    [HarmonyPatch(typeof(DistantWorlds.UI.UserInterfaceController))]
    ...
    public class ScaledRenderMessageHoverEndPatcher
    {
        ...
        {
            return AccessTools.GetTypesFromAssembly(typeof(DistantWorlds.UI.UserInterfaceController).Assembly)
                .SelectMany(type => type.GetMethods())
                .Where(method => method.ReturnType != typeof(void) && method.Name == nameof(DistantWorlds.UI.UserInterfaceController.HideEmpireMessageDialog))
                .Cast<MethodBase>();
        }

        public static void Postfix(RectangleF ret, bool bol)
        {
